...
struct NozzleOption
{
    // Display label retained for the web view and for backward-compatible persistence.
    ...
    {
        auto it = extruder_diameters.find(extruder_id);
        return it == extruder_diameters.end() ? diameter : it->second;
    }
};
#else
struct NozzleOption
